...
const STATUS_LABELS: Record<string, string> = {
  CHECKED_IN: "Presentes", LATE: "Atrasados", ABSENT: "Ausentes", EXCUSED: "Justificados",
  PENDING: "Pendente", APPROVED: "Aprovada", DENIED: "Negada",
  ALTERNATIVE_PROPOSED: "Alt. Proposta", ALTERNATIVE_ACCEPTED: "Alt. Aceita",
  CREATED: "Criada", IN_PROGRESS: "Em Andamento", READY_FOR_APPROVAL: "Ag. Aprovação",
  CHANGES_REQUESTED: "Revisão", COMPLETED: "Concluída", CANCELLED: "Cancelada", EXPIRED: "Expirada",
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard label="Presentes" value={data.checkedIn} sub={`${data.rates.presence}%`} color="text-green-600" />
...
function RequestsTab({ period, operationId }: { period: string; operationId: string }) {
  ...
  const typeData = Object.entries(data.byType).map(([k, v]) => ({ name: STATUS_LABELS[k] ?? k, total: v })).sort((a, b) => b.total - a.total);
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <StatCard label="Tempo médio resp." value={fmtHours(data.avgResponseHours)} />
      </div>
      {typeData.length > 0 && (
        <Card>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <StatCard label="Taxa aprovação" value={fmt(data.approvalRate, "%")} />
        <StatCard label="Taxa retrabalho" value={fmt(data.reworkRate, "%")} color={data.reworkRate !== null && data.reworkRate > 20 ? "text-red-600" : undefined} />
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-3 gap-4">
        ...
        <StatCard label="Tarefas abertas" value={data.byAssignee.reduce((s, a) => s + a.openTasks, 0)} />
        <StatCard label="Tarefas atrasadas" value={data.byAssignee.reduce((s, a) => s + a.overdueTasks, 0)} color="text-red-600" />
      </div>
      <Card>
              ...
              {data.byAssignee.map(a => (
                <TableRow key={a.assigneeId}>
                  ...
                  <TableCell className="text-right">
                    {a.overdueTasks > 0 ? <Badge variant="destructive" className="text-xs">{a.overdueTasks}</Badge> : <span className="text-muted-foreground">0</span>}
                  </TableCell>
                  <TableCell className="text-right">
                    {a.pendingRequests > 0 ? <Badge variant="secondary" className="text-xs">{a.pendingRequests}</Badge> : <span className="text-muted-foreground">0</span>}
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
        ...
        <CardContent>
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={last8} margin={{ left: 0, right: 10, top: 4, bottom: 4 }}>
              ...
              <Line type="monotone" dataKey="presenceRate" stroke="#16a34a" strokeWidth={2} dot={{ r: 3 }} />
            </LineChart>
          </ResponsiveContainer>
        </CardContent>
      </Card>
      <Card>
        <CardHeader><CardTitle className="text-sm">Tarefas e Solicitações</CardTitle></CardHeader>
        <CardContent>
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={last8} margin={{ left: 0, right: 10, top: 4, bottom: 4 }}>
              ...
              <Tooltip />
              <Legend />
              <Bar dataKey="tasksCreated" name="Criadas" fill="hsl(var(--primary))" radius={[3,3,0,0]} />
              <Bar dataKey="tasksCompleted" name="Concluídas" fill="#16a34a" radius={[3,3,0,0]} />
...
export default function SupervisorInsightsPage() {
  ...
  return (
    <AdminLayout title="Insights Operacionais" subtitle="Desempenho da sua operação">
      {/* ── Period selector ── */}
      ...
      {!operationId && (
        <div className="flex items-center gap-2 text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-lg p-4 mb-6">
          <AlertCircle className="w-4 h-4 shrink-0" />
          Nenhuma operação associada à sua conta. Contate o administrador.
        </div>
      )}
      ...
      <Tabs defaultValue="presence">
        ...
        <TabsContent value="presence"><PresenceTab period={period} operationId={operationId} /></TabsContent>
        <TabsContent value="requests"><RequestsTab period={period} operationId={operationId} /></TabsContent>
        <TabsContent value="tasks"><TasksTab period={period} operationId={operationId} /></TabsContent>
        <TabsContent value="workload"><WorkloadTab operationId={operationId} /></TabsContent>
        <TabsContent value="notices"><NoticesTab period={period} operationId={operationId} /></TabsContent>
        <TabsContent value="trends"><TrendsTab operationId={operationId} /></TabsContent>
      </Tabs>
    </AdminLayout>
  );
}
