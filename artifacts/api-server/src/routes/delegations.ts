  ...
  try {
    ...
    (async () => {
      try {
        ...
        const opName = op?.name ?? "operação";
        const respLabels = validResponsibilities.map((r) => RESPONSIBILITY_LABELS[r]).join(", ");
        ...
        // aviso automático não é crítico — não bloqueia a delegação
      }
    })();
