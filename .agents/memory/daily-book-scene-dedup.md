...
- Fora de escopo: a edição manual (PATCH de assignment) ainda permite duplicar.
- A "Conferência por data" do Livro do Show (GET /show-books/:id/resolve) DEVE passar `{ dedupPerScene: true }`, igual ao generate/regenerate. O default da função é desligado; esquecer aqui fazia a conferência mostrar a mesma pessoa em vários papéis da cena (reportado jun/2026). Regra: todo caller de resolveShowBookCast/resolveAssignmentsByRole que mostra atribuições a humanos deve ligar dedupPerScene.
